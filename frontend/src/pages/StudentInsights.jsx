...
import { BarChart3, CalendarDays, Play, RefreshCw, ShieldCheck, Sparkles } from "lucide-react";
...
export default function StudentInsights() {
  ...
  useEffect(() => { let active = true; (async () => {
    try { const response = await apiFetch("/api/support-plans/student"); const data = await response.json().catch(() => ({})); if (response.ok && active) setSupportPlans(data.plans || []); }
    catch { /* A support plan is optional; keep the forecast workspace available. */ }
  })(); return () => { active = false; }; }, [apiFetch]);
