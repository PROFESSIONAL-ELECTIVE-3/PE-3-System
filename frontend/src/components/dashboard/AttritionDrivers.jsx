    ...
    <div className="panel p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {drivers.map((driver, idx) => (
          <div key={idx} className="p-4 rounded-xl border space-y-2" style={{ background: 'var(--sand)', borderColor: 'var(--line)' }}>
            <div className="flex items-center justify-between">
              <span className="eyebrow px-1.5 py-0.5 rounded" style={{ color: 'var(--petrol-dark)', background: 'var(--petrol-light)' }}>
                Rank {String(idx + 1).padStart(2, '0')}
              </span>
              <span className="text-xs font-semibold text-slate-600">{(driver.impact_weight * 100).toFixed(0)}% impact</span>
            </div>
