    ...
    <section className="connected-students" aria-labelledby="connected-students-title">
      ...
      {students.length === 0 ? <div className="connected-students-empty"><UserRound size={20} /><p>No connected students yet. Approve requests in the Students tab to view shared records here.</p></div> : <div className="connected-students-list">
        {students.map(({ connectionId, student, record }) => <article key={connectionId} className="connected-student-card">
          ...
          {!record ? <div className="connected-student-card__empty"><BookOpen size={18} /><p>This student has not submitted their academic information yet.</p></div> : <><div className="connected-student-card__summary"><div><span>Course</span><strong>{record.course}</strong></div><div><span>Attendance</span><strong>{record.attendance === "daytime" ? "Daytime" : "Evening"}</strong></div><div><span>1st semester</span><strong>{record.firstSemesterGrade} / 20</strong></div><div><span>2nd semester</span><strong>{record.secondSemesterGrade} / 20</strong></div></div><details className="connected-student-card__details"><summary><GraduationCap size={15} /> View additional context</summary><div><p><span>Educational special needs</span><strong>{yesNo(record.educationalSpecialNeeds)}</strong></p><p><span>Tuition fees up to date</span><strong>{yesNo(record.tuitionFeesUpToDate)}</strong></p><p><span>Scholarship holder</span><strong>{yesNo(record.scholarshipHolder)}</strong></p></div></details></>}
        </article>)}
      </div>}
    </section>
  );
